...
function train(train_data, valid_data)
 ...
 if opt.train_from:len() == 0 then
  ...
  --(joint) note that reversed the copy time for reverse task
  if opt.joint == 1 then
    decoder_2_clones = clone_many_times(decoder, opt.max_sent_l_src)
    encoder_2_clones = clone_many_times(decoder, opt.max_sent_l_targ)
  end
  -- Reuse memory for the clones(self)
  ...
  function train_batch(data, epoch)
    ...
    for i = 1, data:size() do
      ...
      for t = 1, source_l do
        ...
        rnn_state_enc[t] = out-- save the encoder status for backprop(self)
        context[{{},t}]:copy(out[#out])
      end
      --（joint）use the target as input for joint decoder 
      if opt.joint == 1 then
        for t = 1, target_l do
          encoder_2_clones[t]:training()
          local encoder_input = {target[t], table.unpack(rnn_state_enc_2[t-1])}
          local out = encoder_clones[t]:forward(encoder_input)
      ...
      if opt.joint == 1 then
        for t = 1, source_l do
          ...
          local decoder_input
          if opt.attn == 1 then
            decoder_input = {source[t], context_joint, table.unpack(rnn_state_dec_2[t-1])}
          else
            decoder_input = {source[t], context_joint[{{}, source_l}], table.unpack(rnn_state_dec_2[t-1])}
          end     
      ...
      local drnn_state_dec = reset_state(init_bwd_dec, batch_l) --gradOutput of the decoder
      local drnn_state_dec_2 = reset_state(init_bwd_dec, batch_l)
      local loss = 0
      local loss_2 = 0
      --(self) for each output, forward though the generator, compute loss and add it to the total loss, 
      --backward through the loss criterion calculate the dl_dpred, divide it by batch_l,
      --backward through generator, calculate dl_dtarget, add that to the last drnn_state_dec,
      --create input for decoder,contains target, context(output from the last state of encoder), state of each layer in last step.
      --backward through decoder, calculate dlst
      for t = target_l, 1, -1 do
        ...
        --(self) if not using attention, then only update last status of the encoder_bwd_grads
        if opt.attn == 1 then
        ...
        --(self) zero out the last status of drnn_state_dec    
        drnn_state_dec[#drnn_state_dec]:zero()
        if opt.input_feed == 1 then
          drnn_state_dec[#drnn_state_dec]:add(dlst[3]) --(self) dlst[3] represents the dirivetive of decoder output respect to the input feed
        end	    
        for j = dec_offset, #dlst do --（self）copy the status of decoder grad update in this step for next step
      ...
      if opt.joint == 1 then
        for t = source_l, 1, -1 do
          local pred = generator2:forward(preds_2[t])---TODO make sure this t+1 is alright
          loss_2 = loss_2 + criterion:forward(pred, source[t+1])/batch_l
          local dl_dpred = criterion:backward(pred, source[t+1])
          dl_dpred:div(batch_l)
          local dl_dtarget = generator2:backward(preds[t], dl_dpred)
          ...
          local dlst = decoder_clones[t]:backward(decoder_input, drnn_state_dec_2)
          -- accumulate encoder/decoder grads
          --(self) if not using attention, then only update last status of the encoder_bwd_grads
          if opt.attn == 1 then
            encoder_grads:add(dlst[2])
          ...
          --(self) zero out the last status of drnn_state_dec_2    
          drnn_state_dec_2[#drnn_state_dec_2]:zero()
          if opt.input_feed == 1 then
            drnn_state_dec_2[#drnn_state_dec_2]:add(dlst[3]) --(self) TOUNDERSTAND
          end     
          for j = dec_offset, #dlst do --（self）copy the status of decoder grad update in this step for next step
      ...
      word_vec_layers[2].gradWeight[1]:zero() --(self) TOUNDERSTAND
      --(joint)
      if opt.joint == 1 then
        word_vec_layers[4].gradWeight[1]:zero()
      end
      if opt.fix_word_vecs_dec == 1 then --TODO add handling to joint model
        word_vec_layers[2].gradWeight:zero()
      end

      local grad_norm = 0 --TODO add grad_param of joint model into it
      grad_norm = grad_norm + grad_params[2]:norm()^2 + grad_params[3]:norm()^2

      -- backward prop encoder
      --（self)from last step, encoder_grads and drnn_state_dec were used
      --(self) set the gpu
      if opt.gpuid >= 0 and opt.gpuid2 >= 0 then -- TODO add code for joint model
        cutorch.setDevice(opt.gpuid)
      ...
      local drnn_state_enc = reset_state(init_bwd_enc, batch_l)--gradOutput of the encoder
      local drnn_state_enc_2 = reset_state(init_bwd_enc, batch_l)
      if opt.init_dec == 1 then --(self) use the last(number one for backprop) state of decoder as encoder
        for L = 1, opt.num_layers do
      ...
      --(self) create encoder inputs, backprop through encoder to calculate dlst
      -- then use the current status as the input of next backprop
      for t = source_l, 1, -1 do
        local encoder_input = {source[t], table.unpack(rnn_state_enc[t-1])}
        if opt.attn == 1 then --(self) TOUNDERSTAND
...
-- Create the data loader class.cunn
print('loading data...')
if opt.num_shards == 0 then
...
if opt.train_from:len() == 0 then
  ...
  generator2,_ = make_generator(valid_data, opt)
  
  if opt.brnn == 1 then
    encoder_bwd = make_lstm(valid_data, opt, 'enc', opt.use_chars_enc)
  end
  --(Joint) create joint lstm
  if opt.joint == 1 then
    encoder_2 = make_lstm(valid_data, opt, 'enc', opt.use_chars_enc)
    decoder_2 = make_lstm(valid_data, opt, 'dec', opt.use_chars_enc)
  end
else
...
if opt.joint == 1 then
  tabel.insert(layers,encoder_2)
  tabel.insert(layers,decoder_2)
  tabel.insert(layers,generator2)
end
...
--TODO (joint)set up the gpu for joint model
if opt.gpuid >= 0 then
  for i = 1, #layers do	 
    if opt.gpuid2 >= 0 then 
     if i == 1 or i == 4 then
...
word_vec_layers = {} --the table that contains encoder, decoder and etc..
if opt.use_chars_enc == 1 then
  charcnn_layers = {}
  charcnn_grad_layers = {}
end
