...
function train(train_data, valid_data,train_data_2,valid_data_2)
  ...
  local rnn_state_dec_2 = reset_state(init_fwd_dec, batch_l_2, 0)
  ...
  local drnn_state_dec_2 = reset_state(init_bwd_dec, batch_l_2)
	local loss = 0
  local loss_2 = 0
  ...
  --(joint)
  if opt.joint == 1 then
    for t = target_l_2, 1, -1 do
     local pred_2 = generator_2:forward(preds_2[t])
     loss_2 = loss_2 + criterion:forward(pred_2, target_out_2[t])/batch_l_2
     local dl_dpred = criterion:backward(pred_2, target_out_2[t])
     dl_dpred:div(batch_l_2)
  ...
  if i % opt.print_every == 0 then
      ...
      if opt.joint == 1 then 
        local stats = string.format('Joint Epoch: %d, Batch: %d/%d, Batch size: %d, LR: %.4f, ',
         ...
         stats = stats .. string.format('PPL: %.2f, |Param|: %.2f, |GParam|: %.2f, ',
              math.exp(train_loss_2/train_nonzeros_2), param_norm_2, grad_norm_2)
         stats = stats .. string.format('Training: %d/%d/%d total/source/target tokens/sec',
